...
obj.bind((address[1], int(address[2])))
...
while True:
    try:
        ...
        if js['type'] == 'login':
            ...
            if nickname in user_list:
                obj.sendto(json.dumps({'login': 'fail',
                                       'errormessage': 'the nickname exists'}).encode(),
                           client_address)
            else:
                # 向其他用户发送通知
        ...
        # 私发消息
            ...
            if who not in user_list:
                obj.sendto(json.dumps(
                    {
                        'type': 'message',
                        'message': who + ' not exist or not online.'
                ...
                obj.sendto(json.dumps(
                    ...
                    }).encode(), conn_list[user_list.index(who)])
        ...
        # 离线消息
            ...
            obj.sendto(
                (js['nickname'] + 'offline.').encode(), client_address)
            # 向其他用户发送通知
            for i in range(len(conn_list)):
                obj.sendto(json.dumps(
                    {
