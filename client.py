...
class Client(Cmd):
    ...
    def __receive_message_thread(self):
        ...
        '''
                ...
                # 若接收的数据为文件发送请求，则存储文件信息，并显示
                    ...
                    QuestInfo = filename+' '+who+' '+"yes"
                    self.flag =2
                    self.status = QuestInfo
    ...
    def __send_file_thread(self):
        ...
        while filecount * 1024 <= self.sendfilesize:
            self.__socket.sendto(self.file_send.read(1024), self.fileto_addr)
            time.sleep(0.005)
            filecount += 1
    ...
    def do_login(self, args):
        ...
        if obj['login'] == 'success':
            ...
            self.flag=1
            self.status='[Client] 无法登录到聊天室'+obj['errormessage']
    ...
    def do_catusers(self, arg):
        ...
        self.__socket.sendto(catmessage.encode(), self.currentHost)
    ...
    def do_catip(self, args):
        ...
        self.__socket.sendto(catipmessage.encode(), self.currentHost)
    ...
    def do_help(self, arg):
        ...
        if command == '':
            ...
            print(
                '[Help] getfile filename who yes/no - 接收文件，filename 为文件名,who为发送者，yes/no为是否接收')
        elif command == 'login':
            print('[Help] login nickname - 登录到聊天室，nickname是你选择的昵称')
        elif command == 'send':
            print('[Help] send message - 发送消息，message是你输入的消息')
